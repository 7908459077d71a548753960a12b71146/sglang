...
def _apply_dsa_interleave_half_rope(
    ...
    # Graph-capture safety: the capture flow warms up run_once() twice on
    # the SAME forward_batch before capturing. The first warmup latches
    # rope_cache with warmup-time positions; the capture run then takes
    # the cached branch, so the index_select(positions) compute chain is
    # NEVER recorded into the graph — every replay rotates q_pe/k_pe with
    # frozen warmup-time angles (k_rope diverges ~65% while k_nope stays
    # bit-exact). Recompute while capturing so the chain IS recorded and
    # replays with the live positions buffer. Eager keeps the per-forward
    # latch (fresh ForwardBatch each step).
    _capturing = False
    try:
